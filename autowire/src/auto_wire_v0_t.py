...
def parse_verilog(file_name):
    ...
    defined_signals = np.concatenate(defined_signals)
    defined_signals = np.append(defined_signals, VERILOG_KEYWORDS)
    undefined_signals = np.setdiff1d(signals, defined_signals)
    print(f"signals: {signals}")
    print(f"defined_signals: {defined_signals}")
    print(f"undefined_signals: {undefined_signals}")
    ...
    #     if signal not in defined_signals and signal not in VERILOG_KEYWORDS:
...
def main():
    ...
    if len(undefined_signals):
        print(f"Found {len(undefined_signals)} undefined signals:")
