...
def parse_verilog(file_name: str) -> List[str]:
    ...
    # 信号定义模式
    define_patterns = [
        re.compile(r'\b(?:wire|reg|logic)\s+(?:\[\d+:\d+\])?\s*(\w+)'),
        re.compile(r'\binput\s+(?:\[\d+:\d+\])?\s*(\w+)'),
        re.compile(r'\boutput\s+(?:\[\d+:\d+\])?\s*(\w+)'),
        re.compile(r'\binout\s+(?:\[\d+:\d+\])?\s*(\w+)')
    ]
    ...
    # 6. 按照原始顺序筛选未定义信号
    undefined_signals = []
    exclude_set = VERILOG_KEYWORDS | defined_signals | module_names | set_instance_names | module_instance_signals
...
def generate_signal_definitions(undefined_signals: List[str]) -> List[str]:
